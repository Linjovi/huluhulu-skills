...
def subject_bbox(frame, white_bg, color_threshold=35):
    ...
    if white_bg:
        is_fg = lambda x, y: _color_distance(px[x, y][:3], WHITE) > color_threshold
    else:
        is_fg = lambda x, y: _is_foreground_border(
...
def align_frames(frames):
    ...
    ref_frame = frames[next(i for i, bb in enumerate(bboxes) if bb is not None)]
    ...
    for frame, bbox in zip(frames, bboxes):
        ...
        aligned.append(_shift_frame(frame, dx, dy, white_bg, ref_frame=ref_frame))
    return aligned
